...
- [kvasir](https://github.com/kvasir-legal/kvasir-mcp) - **[🇩🇪 🇦🇹 🇨🇭 🇪🇺 DACH/EU]** Hosted MCP server (OAuth 2.1 or API key) over German federal and state law (Bavaria, NRW, Saxony, Brandenburg, Bremen), Austrian, Swiss and EU law — ~460k norms and ~512k court decisions (German federal courts, Bavaria, NRW, the Austrian supreme courts, CJEU), with 1.4M typed citation edges. Adds citation verification on top of retrieval: `check_draft` checks the citations and quoted wording a model produced against the official text and flags repealed or about-to-change provisions. Sub-units addressable to the sentence; sources checked daily. Free tier.
